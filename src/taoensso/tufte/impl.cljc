...
(let [migrate        {:n-calls :n, :total :sum} ; For back-compatibility
      ...
      format-column  (fn [column] (enc/have format-column? (get migrate column column)))]
...
;; - Handling  relevant for `profiled` only

(enc/defonce ^:dynamic *sig-handlers* "?[<wrapped-handler-fn>]" nil)

(defrecord ProfilingSignal
  ;; Fields based on `taoensso.telemere.impl/Signal`
  [schema inst location, id level sample-rate ctx data, body-result pstats format-pstats-fn]
  Object (toString [psig] (str "taoensso.tufte.ProfilingSignal" (into {} psig))))

(defrecord WrappedProfilingSignal [ns id level psig_]
...
(do     (enc/def-print-impl [x ProfilingSignal] (str "#taoensso.tufte.ProfilingSignal"      (pr-str (into {} x)))))
#?(:clj (enc/def-print-dup  [x ProfilingSignal] (str "#taoensso.tufte.impl.ProfilingSignal" (pr-str (into {} x)))))
...
(defn psig-level-fn [level-or-fn]
  (if-let [x level-or-fn]
    (if (fn? x) x (fn [psig-level] x))
    (do           (fn [psig-level] psig-level))))

(comment ((psig-level-fn :info) :debug))
